...
describe("Admin Pagination", () => {
  ...
  it("다음 버튼을 클릭하면 다음 페이지로 onPageChange를 호출한다", async () => {
    const user = userEvent.setup();
    const handlePageChange = vi.fn();
    ...
    await user.click(screen.getByRole("button", { name: "다음 페이지로 이동" }));
  ...
  it("getHref가 있으면 link로 렌더링한다", () => {
    render(
      <Pagination
        meta={createMeta({ page: 2, totalPages: 5 })}
        getHref={(page) => `/admin/contents?page=${page}`}
      />,
    );

    const link = screen.getByRole("link", { name: "3 페이지로 이동" });

    expect(link).toHaveAttribute("href", "/admin/contents?page=3");
  });
  ...
  it("하위 컴포넌트를 직접 조합할 수 있다", () => {
    render(
      <PaginationList data-testid="list">
        ...
        <PaginationItem>
          <PaginationLink href="/admin?page=2">2</PaginationLink>
        </PaginationItem>
        <PaginationItem>
    ...
    expect(screen.getByRole("link", { name: "2" })).toHaveAttribute("href", "/admin?page=2");
